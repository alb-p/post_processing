...
def apply_pp_technique(technique, model_instance, best_class_thresh, dataset, dataset_pred, unprivileged_groups, privileged_groups, sensible_attribute):
    ...
    if "reject option" in technique_name:
        ...
        df_pred = dataset_pred.convert_to_dataframe()[0]
        X = df_pred.drop(['income'], axis=1)
        y = df_pred['income']
        y.astype(int)
        sex_array = df_pred['sex']
        technique_instance = technique_instance.fit(X, y, sensitive_features=sex_array)
        already_fitted = True
    else:
    ...
    if not already_fitted:
        ...
        TO_transf_test_pred = technique_instance.predict(X, sensitive_features=sex_array)
